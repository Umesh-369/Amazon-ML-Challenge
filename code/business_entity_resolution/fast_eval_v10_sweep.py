...
def main():
    ...
    print("Building Inverted Indices for v10 Parameter Sweep...")
    ...
    print(f"Indices built in {time.time()-t0:.1f}s. Precomputing candidate scores for all validation entities...")

    # Pre-collect all candidate scores per entity so threshold sweep runs in <1 second!
    candidate_pools = []
    for row in val_s1:
        s1_id = row[0]
        country = fast_norm(row[3]) if len(row) > 3 else ""
        ...
        for c_eid, c_num, c_post, c_metro in exact_list:
            ...
            if score > 0:
                scored_candidates[c_eid] = max(scored_candidates.get(c_eid, 0), score)
        ...
        # 4. Building Number + Brand Prefix
        if b_num > 0 and len(norm_name) >= 4:
            ...
            if 0 < len(p_list) <= 6:
                for c_eid, c_stok, c_post, c_metro, c_toks, c_3g in p_list:
                    ...
                    if s_tok and c_stok and s_tok == c_stok:
                        score = 92 if (jacc >= 0.25 or g_jacc >= 0.35) else 84
                    elif jacc >= 0.30 or g_jacc >= 0.40:
                        score = 88
                    if score > 0:
                        scored_candidates[c_eid] = max(scored_candidates.get(c_eid, 0), score)

        # 5. Address Anchor Matches
        if b_num > 0 and s_tok:
            ...
            if a_bkt == 1:
                ...
                if not (metro and c_metro and metro != c_metro) and not (postcode and c_post and postcode != c_post):
                    jacc = token_jaccard(s1_toks, c_toks)
                    g_jacc = char_3gram_jaccard(s1_3grams, c_3g)
                    if jacc >= 0.25 or g_jacc >= 0.35 or (first_char and c_fchar and first_char == c_fchar and g_jacc >= 0.25):
                        scored_candidates[c_eid] = max(scored_candidates.get(c_eid, 0), 84)

        # 6. Distinctive Name Token + Street Token
        if s_tok:
                ...
                if 0 < len(ts_list) <= 4:
                    for c_eid, c_toks, c_metro, c_post, c_3g in ts_list:
                        ...
                        if jacc >= 0.30 or g_jacc >= 0.40:
                            scored_candidates[c_eid] = max(scored_candidates.get(c_eid, 0), 90)
        ...
        if postcode and len(norm_name) >= 4:
            ...
            if 0 < len(pin_list) <= 3:
                for c_eid, c_toks, c_3g in pin_list:
                    ...
                    g_jacc = char_3gram_jaccard(s1_3grams, c_3g)
                    if jacc >= 0.30 or g_jacc >= 0.40:
                        scored_candidates[c_eid] = max(scored_candidates.get(c_eid, 0), 88)

        true_m = gt_dict.get(s1_id, set())
        candidate_pools.append((s1_id, scored_candidates, true_m))

    print("\n" + "=" * 75)
    print("THRESHOLD SWEEP ON MACRO F0.5 (PRECISION VS RECALL OPTIMIZATION):")
    print("=" * 75)
    ...
    for threshold in [74, 76, 78, 80, 82, 84, 86, 88]:
        precisions = []
        recalls = []
        ...
        for s1_id, scored_candidates, true_m in candidate_pools:
            scored_s2 = []
            scored_s3 = []
            for cid, sc in scored_candidates.items():
                if sc >= threshold:
        ...
        print(f"Threshold >= {threshold}:  Precision = {macro_p:.4f} | Recall = {macro_r:.4f} | F0.5 = {f05:.6f} | FPs = {total_fp:5d} | FNs = {total_fn:5d}")

        if f05 > best_f05:
            best_f05 = f05
            best_thresh = threshold
            best_metrics = (macro_p, macro_r, total_tp, total_fp, total_fn, correct_singletons)

    print("=" * 75)
    print(f"Optimal Threshold: {best_thresh} -> Best Macro F0.5 = {best_f05:.6f} (P={best_metrics[0]:.4f}, R={best_metrics[1]:.4f})")
    print(f"Delta over v06 (0.799943): {best_f05 - 0.799943:+.6f}")
    print("=" * 75)
